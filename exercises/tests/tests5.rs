...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller is responsible for ensuring that the `address` 
    // is valid and points to a mutable `u32` that is properly aligned 
    // and not dangling. We assume that the contract of the function 
    // has been honored by the caller.
    let ptr = address as *mut u32; // Convert usize back to a mutable pointer
